...
1. **对抗性悲观心智与置疑假设清单机制 (Adversarial Pessimism & Assumptions Challenge Check)**：
   - 严禁假定外部调用客户端发送的 `MessageSequence` 参数是良性的或符合开发文档约定的类型。必须假设攻击者可以完全操纵序列中写入的所有 Parcelable 数据或 ArrayBuffer 字节。
   - 在判定一个 IPC 服务安全或判定某 code 分支无风险前，必须在思考链中强制列出隐含假设并扮演红队进行驳斥（例如：“假设了 `readInterfaceToken` 校验能封死恶意请求” ➜ 红队驳斥：检查外部调用是否可以通过伪造 token 或者在没有 token 校验的 case 分支中调用 ➜ 挑战假设，重新评估漏洞风险）。
2. **先判敏感，再查规则**：非敏感业务跳过，避免在无价值业务上浪费审计
3. **完整流程必须梳理清楚**：输入 → 分发 → 执行 → 输出，四段缺一不可
4. **利用方法必须可执行**：逐步描述，包括如何获取 descriptor、如何构造 data、调用什么 API
5. **实际源码为证据**：所有判断必须有代码原文支撑，不可凭空猜测
6. **规则硬约束与修饰符限制**：AI 必须严格执行 rules/ 下对应规则的 audit_guide 语义步骤进行漏洞确认。只有在代码事实完全符合规则中明确定义的 severity_modifiers 条件时，才允许调整严重级别或跳过不报；严禁 AI 凭主观推测或无代码证据的“安全上下文”自发规避规则校验。
