...
from .common import (canonical_json, ensure_run_dirs, load_capabilities, now,
                     profiles_for_entry, read_json, run_paths, stable_id, write_json)
...
def _candidate_groups(candidates):
    ...
    for candidate in candidates:
        ...
        if component_id:
            ...
            key = canonical_json([
                "module-scope", candidate.get("module_name"), candidate.get("type"),
            ])
...
def initialize_run(run_dir, project_model):
    ...
    with database(paths["db"]) as conn, transaction(conn):
        ...
        for group_key, rows in sorted(_candidate_groups(candidates).items()):
            ...
            entry_types = [row["entry_type"] for row in facets]
            profiles = profiles_for_entry(entry_types, capabilities)
            component = (first.get("component_name")
                         or f"{first.get('module_name') or 'project'} dynamic {first.get('type')}")
            entry_key = canonical_json([
                first.get("module_name"), first.get("component_id") or group_key,
            ])
            ...
            if run["audit_mode"] != "capability" or profiles:
        ...
        append_event(conn, "run_initialized", run["run_id"], {
            ...
            "semantic_analysis_tasks": len(task_ids),
