...
| Component Semantic Analyzer | 使用 Atlas 分析一个组件内的输入、数据传递、实际操作和防护事实；到组件边界时记录参数映射 | 继续分析下游组件、漏洞分类、六维判断、生成报告 |
| Component Correlator（Python） | 连接各组件的参数映射，只保留从真实外部入口到敏感操作的最短可控链 | 理解源码、猜测缺失连接、创建 AI 路径任务 |
...
每个实际进入审计范围的分析单元最多创建一个语义任务，完成组件输入确认、本地操作收集、等价操作归并、组件连接和覆盖记录。全量模式从全部组件开始；能力和组件模式从命令指定的范围开始，并沿已证明保持控制性的 `component_handoffs` 扩展下游组件。没有新的组件后，Python 连接完整参数链。只有起始外部入口可达的本地操作组和跨组件操作组进入六维验证；普通代码分支和组件连接不会生成路径任务。
...
<run_dir>/
  ...
  exports/component_handoffs.json
