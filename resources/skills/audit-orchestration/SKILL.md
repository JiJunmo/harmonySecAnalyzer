...
AI 任务严格分成 `component_semantic_analysis`、`exploitability_validation` 和 `poc_generation`。语义阶段每个组件只有一个持久任务记录和一个探索状态，但同一任务可按单轮节点容量多次派发。Agent 通过 `explore-next` 领取节点，使用 Atlas 完成组件输入确认、数据追踪、实际操作归并，以及组件间参数、身份和权限变化记录，再通过 `explore-record` 即时校验并提交当前节点事实；本轮结束时调用 `explore-finish`，由运行时直接重新排队或确定性生成、校验并落库最终组件语义结果。Agent 不手写轮次结果或完整组件结果。
