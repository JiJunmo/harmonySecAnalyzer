...
def _handle(row, paths):
    ...
    if row["kind"] == "component_semantic_analysis":
        handle["worker_prompt"] = (
            ...
            f"exploration_protocol：循环领取代码范围、核实分支与调用后续、提交步骤，最后必须调用 finish 命令。"
            f"本轮容量不足时用 result=null 的 ranges 保存所有未完成范围，设置 pause_requested=true；"
            f"Atlas 缺边时必须先核实调用点和绑定/分派源码：完整解析只接续，部分解析接续已证明目标并保留剩余 gap，完全无法解析才只写 gap；"
            f"读取 coverage 和 transitions，必要时分页查询前序事实；不得把尚未分析写成 gaps。"
