...
为本次审计创建隔离 run，解析项目配置、建立 Atlas 索引并确定性生成组件分析单元。每个组件只执行一次语义分析，落盘组件内操作、调用触发控制和真实参数映射；全部组件结束后由脚本从已确认外部入口连接敏感操作，再创建六维验证任务。确定性根因归并和准入通过后生成 Markdown 与 HTML 报告。
