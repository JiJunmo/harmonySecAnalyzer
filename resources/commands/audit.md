...
为本次审计创建隔离 run，解析配置、建立 Atlas 索引并生成组件任务。每个组件分轮分析代码范围，先登记局部分支和调用后续，已完成范围批量保存，未完成范围保持待办。跨轮恢复结构化条件与事实；所有已登记范围闭合或形成明确缺口后，脚本生成语义结果，再进行组件连接、六维验证、根因归并和报告生成。协议更新后旧 run 不支持续跑，旧增量基线需重新全量审计。
