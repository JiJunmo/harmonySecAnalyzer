...
为本次审计创建隔离 run，解析项目配置、建立 Atlas 索引并确定性生成组件探索单元。每个组件只有一个持久语义任务记录，但可按上下文容量连续派发多轮短任务；Agent 使用 Atlas 逐节点探索，运行时保存断点并在全部分支闭合后生成最终组件语义结果。全部组件结束后由脚本从已确认外部入口连接敏感操作，再创建六维验证任务。确定性根因归并和准入通过后生成 Markdown 与 HTML 报告。
