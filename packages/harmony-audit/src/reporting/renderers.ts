...
const labels: Record<string, string> = {
  ...
  generated_unverified: "已生成，未编译验证", build_verified: "已通过编译验证", device_verified: "已通过设备验证",
  generation_failed: "生成失败", pending_generation: "生成中",
};
...
function pocMarkdown(finding: Row): string {
  ...
  const assuranceStatus = String(artifact.assurance_status ?? finding.poc_status ?? "");
  if (assuranceStatus) preamble.push(`- 产物状态：**${label(assuranceStatus)}**`);
  ...
  const unavailable = assuranceStatus === "generation_failed" ? "PoC 生成失败" : assuranceStatus === "pending_generation" ? "PoC 正在生成" : "未生成 PoC";
...
export function renderMarkdown(report: Row): string {
  ...
  const lines: string[] = [
    ...
    `- 未形成可利用路径：${classification.no_exploitable_path ?? 0}`,
...
export function renderHtml(report: Row): string {
  ...
  const findingHtml = findings.length ? findings.map((finding, index) => {
    ...
    const pocStatus = String(poc.assurance_status ?? finding.poc_status ?? "");
    ...
    const pocUnavailable = pocStatus === "generation_failed" ? "PoC 生成失败" : pocStatus === "pending_generation" ? "PoC 正在生成" : "未生成 PoC";
    const pocStatusHtml = pocStatus ? `<dt>产物状态</dt><dd>${badge(pocStatus)}</dd>` : "";
    const pocHtml = pocCodeText ? `<details open><summary>查看验证方式 / PoC <b class="badge ${escapeHtml(finding.severity)}">${escapeHtml(poc.entry_type || "PoC")}</b></summary><dl class="kv">${pocStatusHtml}${pocMeta ? `<dt>入口 / 触发 / 语言</dt><dd>${pocMeta}</dd>` : ""}${pocExpected ? `<dt>预期现象</dt><dd>${escapeHtml(pocExpected)}</dd>` : ""}${pocLimitations ? `<dt>复现限制</dt><dd>${escapeHtml(pocLimitations)}</dd>` : ""}${pocDevice && pocDevice !== "none" ? `<dt>复现环境</dt><dd>${escapeHtml(label(pocDevice))}</dd>` : ""}</dl>${pocPrereqs ? `<h5>前置条件</h5><ul class="notes">${pocPrereqs}</ul>` : ""}${pocSteps ? `<h5>逐步复现</h5><ol class="notes">${pocSteps}</ol>` : ""}${pocSymbols ? `<h5>引用核验</h5><div class="facts">${pocSymbols}</div>` : ""}<pre><code>${escapeHtml(pocCodeText)}</code></pre></details>` : `<details><summary>查看验证方式 / PoC ${pocStatus ? badge(pocStatus) : ""}</summary><pre><code>${escapeHtml(pocUnavailable)}</code></pre></details>`;
...
.badge.no_exploitable_path,.badge.build_verified,.badge.device_verified{color:var(--green);border-color:#b8d2c5;background:#f1f8f4}.badge.generated_unverified,.badge.pending_generation{color:var(--orange);border-color:#e5c39f;background:#fff8ef}.badge.generation_failed{color:var(--red);border-color:#e8bbb6;background:#fff4f2}
