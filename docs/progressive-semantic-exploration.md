...
flowchart LR
    ...
    C --> D["派发组件探索轮次"]
    D --> E["Agent 领取待分析节点"]
    E --> F["Agent 调用 Atlas MCP"]
    F --> G["运行时校验并保存节点、连接和安全事实"]
    G --> H{"本轮容量是否用完"}
    H -- "否" --> E
    H -- "是" --> I["结束当前轮次"]
    I --> J{"是否仍有待分析节点"}
...
当本轮容量用完或没有待分析节点时调用。脚本根据数据库状态生成格式固定的轮次 submission，Agent 不手工拼接大型最终 JSON。
...
| `resources/skills/audit-orchestration/config/schemas/component-exploration-step.schema.json` | 单节点探索步骤契约 |
...
1. 构造超过 50 层的线性调用链，不设置深度上限，能够跨多轮完整收口。
