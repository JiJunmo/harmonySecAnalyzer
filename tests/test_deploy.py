...
class DeployRenderTest(unittest.TestCase):
    def test_opencode_subagents_can_use_runtime_result_protocol(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("commands.record", semantic)
            self.assertIn("commands.context", semantic)
            self.assertIn("coverage", semantic)
            self.assertIn("wait_for", semantic)
            self.assertNotIn("successor.state", semantic)
            self.assertNotIn("resume_from", semantic)
    ...
    def test_claude_render_produces_expected_artifacts(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("pause_requested", semantic)
    ...
    def test_local_render_is_self_contained_and_cwd_independent(self):
        with tempfile.TemporaryDirectory() as td, tempfile.TemporaryDirectory() as cwd:
            ...
            self.assertNotIn("resume", schema["properties"])
            self.assertIn("ranges", schema["required"])
            self.assertIn("transitions", schema["required"])
            self.assertIn("checked", schema["$defs"]["result"]["required"])
            self.assertIn("wait_for", schema["$defs"]["range"]["required"])
            self.assertNotIn("resource_limit", schema["$defs"]["termination"]["properties"]["kind"]["enum"])
            self.assertTrue((tree / "skills/audit-orchestration/scripts/audit_runtime/exploration_context.py").is_file())
