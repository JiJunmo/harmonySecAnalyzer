...
渐进探索先登记局部分支和调用后续，再按代码范围分析。已完成和待办范围统一保存，一轮可以处理多个分支；容量不足时通过 pause_requested 换上下文，不把未分析代码当作解析失败。

跨轮输入包含范围进度、进入条件、已有操作组、停止依据和调用返回位置，完整记录可分页查询。系统或第三方边界只停止内部展开，调用者后续仍需分析。所有已登记范围闭合后才生成语义结果；这不代表编译器级分支穷尽证明。

运行协议版本 25：更新后需新建审计，旧 run 无法继续；旧契约的增量基线需先重新全量审计。详细说明见 [范围探索协议](docs/progressive-semantic-exploration.md)。
