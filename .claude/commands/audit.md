...
argument-hint: <repo-path> [--incremental] [--resume <run-dir>] [--capability <CAP-ID>] [--component <Name>]
---
...
执行方式：使用 Agent 工具派发编排者，`subagent_type: harmony-auditor`，`description: "运行 /audit 审计编排"`，`prompt` 原样传入完整参数 `$ARGUMENTS`（例如 `<repo-path>` 或 `--incremental <repo-path>`，保留所有标志与位置参数），等待其完成并向用户汇报结果。
