...
description: 基于 SQLite 的组件级安全分析运行时调用协议。当需要执行审计运行时的 prepare/claim-batch/reconcile-batch/finalize/resume/status 命令，或确认 run 状态机、准入条件、增量/能力/组件模式行为与 run 目录结构时使用。
---
